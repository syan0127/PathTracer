...
void JSONReader::LoadSceneFromFile(QFile &file, const QStringRef &local_path, Scene &scene)
{
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        foreach(const QJsonValue &frame, frames) {
            ...
            if(sceneObj.contains(QString("csgs"))) {
                csgList = sceneObj["csgs"].toArray();
                foreach(const QJsonValue &csgVal, csgList){
                    QJsonObject csgObj = csgVal.toObject();
                    LoadCSGs(csgObj, mtl_name_to_material, local_path, &scene.primitives, &scene.drawables);
                }
            }
        }

        //commented out for CSGs
...
//        }
        ...
        //iterate over drawables - for CSG
...
bool JSONReader::LoadGeometry(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives, QList<std::shared_ptr<Drawable>> *drawables)
{
    ...
    else if(QString::compare(type, QString("Implicit")) == 0) //for Implicit surface
    {
        shape = std::make_shared<ImplicitSurface>();
        ((ImplicitSurface*) shape.get())->surface = geometry["surface"].toInt();
    }
    else
    {
    ...
    (*drawables).append(shape); //added for CSG
    return true;
}
...
bool JSONReader::LoadLights(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives, QList<std::shared_ptr<Light>> *lights, QList<std::shared_ptr<Drawable>> *drawables)
{
    ...
    (*drawables).append(primitive->shape); //added for CSG
    return true;
}
...
bool JSONReader::LoadCSGs(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives, QList<std::shared_ptr<Drawable>> *drawables)
{
    QJsonArray operatorList;
    if(geometry.contains(QString("operators"))) operatorList = geometry["operators"].toArray();
    std::vector<Operator> operators;

    foreach(const QJsonValue &opVal, operatorList){
        QString oper = opVal.toString();
        if(QString::compare(oper, QString("shape")) == 0) {
            operators.push_back(SHAPE);
        } else if(QString::compare(oper, QString("union")) == 0) {
            operators.push_back(UNION);
        } else if(QString::compare(oper, QString("diff")) == 0) {
            operators.push_back(DIFF);
        } else if(QString::compare(oper, QString("inter")) == 0) {
    ...
    QJsonArray transformList;
    if(geometry.contains(QString("transforms"))) {
        transformList = geometry["transforms"].toArray();
    }
    QJsonArray materialList;
    if(geometry.contains(QString("materials"))) {
        materialList = geometry["materials"].toArray();
    }
    QJsonArray shapeList;
    if(geometry.contains(QString("shapes"))) {
        shapeList = geometry["shapes"].toArray();
    }
    QJsonArray nameList;
    if(geometry.contains(QString("names"))) {
        nameList = geometry["names"].toArray();
    }

    QJsonArray surfaceList;
    if(geometry.contains(QString("surfaces"))) {
        surfaceList = geometry["surfaces"].toArray();
    }
    std::vector<std::shared_ptr<Primitive>> primitiveList;

    int i = 0, k = 0;
    QMap<QString, std::shared_ptr<Material>>::iterator j;

    foreach(const QJsonValue &shapeVal, shapeList){
        std::shared_ptr<Shape> shape = nullptr;
        ...
        {
            shape = std::make_shared<ImplicitSurface>();
            ((ImplicitSurface*) shape.get())->surface = surfaceList.at(k).toInt();
            k++;
        }
        else
        {
        ...
        for (j = mtl_map.begin(); j != mtl_map.end(); ++j) {
            if(j.key() == material_name){
                primitive->material = j.value();
            }
        }
        primitive->name = nameList.at(i).toString();
        shape->transform = LoadTransform(transform);
        primitiveList.push_back(primitive);
        i++;
    }
    std::shared_ptr<CSG> csg = std::make_shared<CSG>(primitiveList, operators);
    if(geometry.contains(QString("name"))) {
        csg->name = geometry["name"].toString();
    }
    (*primitives).append(csg);
    foreach (std::shared_ptr<Primitive> prim, csg->primitives) {
     (*drawables).append(prim->shape);
    }
    return true;
}
