...
void MyGL::RenderScene()
{
    ...
    {
#define PM_PP_MULTITHREAD
#ifndef PM_PP_MULTITHREAD
        std::vector<std::vector<Photon>>* photonSets = new std::vector<std::vector<Photon>>(1);
        PhotonMapper* pm = new PhotonMapper(numPhotons, &((*photonSets)[0]), &scene, sampler->Clone(0), recursionLimit);
        ...
        for(int i = 0; i < 8; ++i)
        {
            ...
            int seed = i;
            PhotonMapper* pm = new PhotonMapper(threadPhotonCount, &((*photonSets)[i]), &scene, sampler->Clone(seed), recursionLimit, &kdTree, photonSearchRadius);
